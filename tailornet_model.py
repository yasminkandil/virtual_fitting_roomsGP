...
from TailorNet_master.trainer.lf_trainer import get_best_runner as lf_runner
from TailorNet_master.trainer.hf_trainer import get_best_runner as hf_runner
from TailorNet_master.trainer.ss2g_trainer import get_best_runner as ss2g_runner
from TailorNet_master.dataset.canonical_pose_dataset import ShapeStyleCanonPose
...
def get_best_runner(garment_class='t-shirt', gender='female', lf_logdir=None, hf_logdir=None, ss2g_logdir=None):
    ...
    if lf_logdir is None:
        lf_logdir = os.path.join(TailorNet_master.global_var.MODEL_WEIGHTS_PATH, "{}_{}_weights/tn_orig_lf".format(garment_class, gender))
    if hf_logdir is None:
        hf_logdir = os.path.join(TailorNet_master.global_var.MODEL_WEIGHTS_PATH, "{}_{}_weights/tn_orig_hf".format(garment_class, gender))
    if ss2g_logdir is None:
        ss2g_logdir = os.path.join(TailorNet_master.global_var.MODEL_WEIGHTS_PATH, "{}_{}_weights/tn_orig_ss2g".format(garment_class, gender))
