...
from TailorNet_master.utils.diffusion_smoothing import numpy_laplacian_uniform as laplacian


def get_fn_vn(vert, faces):
    fn = TriNormals(v=vert, f=faces).reshape((-1, 3))
    vn = VertNormals(v=vert, f=faces).reshape((-1, 3))
    return vn,fn
