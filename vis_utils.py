...
def get_amass_sequence_thetas(which):
    ...
    seqs = np.load('TailorNet_master/visualization/amass_sequences.npy', allow_pickle=True, encoding='latin1').item()
...
def get_specific_pose(which):
    ...
    0 - apose
    """
    thetas = np.load('TailorNet_master/visualization/some_thetas.npy')
    which = int(which)
    return thetas[which]
